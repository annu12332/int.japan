...
const INTJapanRegulationsPage = ({ lang = 'en' }) => {
    // translations object thik ager motoi thakbe
    const translations = {
        en: {
            ...
            labels: {
                ...
                guaName: "Guarantor's name (English)", relation: "Relationship with the person",
        ...
        bn: {
            ...
            labels: {
                ...
                guaName: "জামিনদারের নাম (ইংরেজি)", relation: "আবেদনকারীর সাথে সম্পর্ক",
    ...
    const InputLine = ({ label, width = "w-full", placeholder = "" }) => (
        <div className="flex items-end gap-2 mb-4">
            <span className="text-[12px] font-bold text-slate-700 whitespace-nowrap">{label}</span>
            <input type="text" placeholder={placeholder} className={`${width} border-b border-slate-900 outline-none px-2 py-0 text-[13px] font-medium bg-transparent`} />
        </div>
    );

    return (
        <div className="bg-slate-50 min-h-screen py-10 px-4">
            {/* Language Shifter Bad deya hoyeche ekhane */}

            <div className="max-w-[850px] mx-auto bg-white shadow-xl p-8 md:p-12 border border-slate-200 print:shadow-none print:border-none">
                
                {/* Main Header */}
                <h1 className="text-xl md:text-2xl font-black text-[#1A3673] text-center uppercase tracking-tight leading-tight mb-6">
                    {t.title}
                </h1>

                {/* Important Alert */}
                <div className="bg-[#FF9F9F] text-white py-2 px-6 rounded-full text-center mb-8">
                    <p className="text-[11px] md:text-[13px] font-bold italic">{t.important}</p>
                </div>
                ...
                <div className="space-y-2 mb-10">
                    ...
                    {t.rules.map((rule, i) => (
                        <div key={i} className="flex gap-4 p-2 bg-slate-50 border border-slate-100 rounded-lg group hover:bg-white transition-all">
                            <span className="flex-shrink-0 w-7 h-7 bg-green-600 text-white rounded-full flex items-center justify-center text-[10px] font-bold">
                                {(i + 1).toString().padStart(2, '0')}
                            </span>
                            <p className="text-[11px] md:text-[12.5px] text-slate-700 font-semibold self-center">{rule}</p>
                        </div>
                    ))}
                </div>

                {/* Pledge Section */}
                <div className="border-2 border-dashed border-green-600 p-6 rounded-2xl bg-green-50/30 text-center mb-10">
                    <h3 className="text-lg font-black text-green-800 uppercase mb-2">{t.pledgeTitle}</h3>
                    <p className="text-[12px] font-bold text-slate-600">{t.pledgeText}</p>
                </div>

                {/* Personal Information Fields */}
                <div className="space-y-6 mb-12">
                    <div className="flex flex-wrap gap-4">
                        <InputLine label={t.labels.subDate} width="w-20" />
                        <InputLine label={t.labels.year} width="w-12" />
                        <InputLine label={t.labels.month} width="w-12" />
                        <InputLine label={t.labels.day} width="w-12" />
                    </div>
                    ...
                    <div className="flex flex-wrap gap-6 items-center">
                        <div className="flex gap-2">
                           <InputLine label={t.labels.dob} width="w-20" />
                           <InputLine label={t.labels.year} width="w-12" />
                           <InputLine label={t.labels.month} width="w-12" />
                           <InputLine label={t.labels.day} width="w-12" />
                        </div>
                        <div className="flex gap-4 items-center">
                            ...
                            <label className="text-xs font-bold flex items-center gap-1"><input type="radio" name="g" /> {t.labels.male}</label>
                            <label className="text-xs font-bold flex items-center gap-1"><input type="radio" name="g" /> {t.labels.female}</label>
                        </div>
                    </div>
                    ...
                    <InputLine label={t.labels.curAddr} placeholder="Guarantor Address" />
                ...
                {/* Footer Box */}
                <div className="bg-sky-50 p-6 rounded-3xl border border-sky-100 mb-8">
                    <h4 className="text-[#1A3673] font-black text-sm mb-3 uppercase">{t.footerTitle}</h4>
                    <div className="space-y-2 text-[11px] text-slate-600 font-bold">
                        <p className="flex items-center gap-2"><Mail size={14} className="text-red-500" /> Email: dhaka@int-japan.com</p>
                        <p className="flex items-center gap-2 text-xl text-[#1A3673]"><Phone size={22} className="text-pink-600" /> Tel : 019-4234-8132</p>
                        <p className="flex items-center gap-2"><MapPin size={14} className="text-red-600" /> Address: House No.27/1(Lift-8), Road No.02, Sector No.15, Uttara, Diabari, Dhaka-1230</p>
                    </div>
                </div>

                {/* Print Buttons */}
                <div className="flex gap-4 print:hidden">
                    <button onClick={() => window.print()} className="bg-green-600 text-white px-8 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-green-700 shadow-md transition-all active:scale-95">
                        <Printer size={18} /> {t.print}
                    </button>
                    <button className="bg-sky-500 text-white px-8 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-sky-600 shadow-md transition-all active:scale-95">
                        <Send size={18} /> {t.send}
                    </button>
                </div>
            </div>
